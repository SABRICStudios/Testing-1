// select_ellipse.js - Elliptical & Circular Region Selection Engine
(function() {
    let startX = 0, startY = 0;

    function onMouseDown(e) {
        if (window.imgState?.selection?.mode !== 'ellipse') return;
    ...
    function onMouseMove(e) {
        if (!window.SelectionManager?.isSelecting || window.imgState?.selection?.mode !== 'ellipse') return;
    ...
    function onMouseUp() {
        if (window.imgState?.selection?.mode === 'ellipse' && window.SelectionManager.isSelecting) {
